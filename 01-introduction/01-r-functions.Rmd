...
f <- function(x) {
  y <-x
  for(i in seq_along(x)){y[i] <- prod(subset(x, x != x[i]))}
return(y)
  }
...
y[i] <- if((as.numeric(x[i])%%3 == 0) & (as.numeric(x[i])%%5 == 0 )) {x[i] = 'FizzBuzz'}
    else{if(as.numeric(x[i])%%3 == 0){x[i] = 'Fizz'}
      else{if(as.numeric(x[i])%%5 == 0){x[i] = 'Buzz'}
          else{x[i]}
        }
    }
}
